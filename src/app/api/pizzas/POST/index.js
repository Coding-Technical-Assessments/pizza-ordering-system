...
export const handlePOST = async (request) => {
  try {
    ...
    var formattedPizzaCreateData = formatPizzaCreateData(body);

    try {
      await pizzaSchema.validate(formattedPizzaCreateData, {
        abortEarly: false,
      });
    ...
    const newPizza = await prismaClient.pizza.create({
      data: {
        ...
        price: formattedPizzaCreateData.price,
        image: formattedPizzaCreateData.image,
